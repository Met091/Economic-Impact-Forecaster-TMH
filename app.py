...
# --- Page Configuration ---
st.set_page_config(
    page_title="Economic Forecaster V12 (AV Historical)",
    page_icon="🇺🇸", # US flag for AV data focus
    layout="wide",
...
with st.sidebar:
    ...
    # ... (Date Range, Timezone, Currency, Impact filters remain the same as V10/V11) ...
...
# --- Load Economic Calendar Data (investpy) ---
# st.info message about fetching moved to after date selection in sidebar for clarity
...
with st.sidebar:
    ...
    if not economic_df_master.empty and 'Impact' in economic_df_master.columns:
        ...
        for opt in impact_level_options_std:
            if opt in data_impact_values and opt not in combined_impact_options: combined_impact_options.append(opt)
        for opt in data_impact_values:
            if opt not in combined_impact_options: combined_impact_options.append(opt)
...
st.title("🇺🇸 Economic Impact Forecaster V12 (AV Historical)")
st.markdown(f"""
Configure filters in the sidebar. Calendar data from **Investing.com (via `investpy`)** for period **{st.session_state.start_date_filter.strftime('%b %d, %Y')}** to **{st.session_state.end_date_filter.strftime('%b %d, %Y')}**.
Historical trends for select US indicators powered by **Alpha Vantage**.
""")
if 'ALPHA_VANTAGE_API_KEY' not in st.secrets:
    st.warning("Alpha Vantage API key not configured in secrets. Real US historical data will not be available.")
if economic_df_master.empty:
    st.error(f"🚨 Failed to load economic calendar data using `investpy`. Please check network or try different dates.")
...
if economic_df_master.empty: pass 
    ...
    st.warning("⚠️ No economic events match the selected filters. Please adjust your selections.")
else:
    # ... (Event Selection, Details, Tabs logic remains largely the same as V11) ...
    # Key change will be in Tab 2 for historical data source indication.
    ...
    with col_event_selection:
        ...
        if 'Timestamp' in economic_df_filtered.columns and pd.api.types.is_datetime64_any_dtype(economic_df_filtered['Timestamp']):
            economic_df_filtered['display_name'] = economic_df_filtered.apply(
                lambda row: (f"{convert_and_format_time(row['Timestamp'], selected_tz_name)} - "
                             f"{row.get('Currency','N/A')} - {row.get('Impact','N/A')} - {row.get('EventName','Unknown Event')}")
                if pd.notna(row.get('EventName')) else f"Invalid Event Data @ {convert_and_format_time(row.get('Timestamp'), selected_tz_name)}", axis=1)
        else: 
            economic_df_filtered['display_name'] = economic_df_filtered.apply( lambda row: f"Data Error - {row.get('Currency','N/A')} - {row.get('EventName','Unknown Event')}", axis=1)
    ...
    with tab1: # Interpretation Tab
        ...
        outcome_color_map = {"Bullish": "#1E4620", "Bearish": "#541B1B", "Consolidating": "#333333", "Qualitative": "#2E4053", "Indeterminate": "#4A235A", "Error": "#641E16"}
    ...
    with tab2: # Historical Trends Tab
        ...
        df_hist = load_historical_data(event_name_str) # This now tries Alpha Vantage
        if not df_hist.empty:
            # Determine if data is from AV (only 'Actual') or sample ('Actual', 'Forecast', 'Previous')
            if 'Forecast' not in df_hist.columns and 'Previous' not in df_hist.columns and 'Actual' in df_hist.columns:
                st.caption(f"Displaying real historical 'Actual' values from Alpha Vantage for {event_name_str} (US Data). Forecast/Previous not available for historical points from this source.")
            else:
                st.caption(f"Displaying sample historical data for {event_name_str} (includes sample Actual, Forecast, Previous).")
            ...
            st.info(f"No historical data (neither Alpha Vantage nor sample) found for '{event_name_str}'.")
    with tab3: # Simulate Actual Release Tab
        ...
        unit_sim, step_value = indicator_props_sim.get("unit", ""), (0.1 if "%" in indicator_props_sim.get("unit", "") else 1.0 if "K" in indicator_props_sim.get("unit", "") else 0.01)
    ...
    with st.expander("🗓️ Full Economic Calendar Overview (Filtered - Data via investpy)", expanded=False):
        if not economic_df_filtered.empty:
            # ... (Calendar display logic as before) ...
    ...
    st.markdown("---")
    st.caption("Disclaimer: Generalized interpretations, not financial advice. Calendar: investpy (unofficial). US Historicals: Alpha Vantage (free tier limits apply). Other historicals: sample. Data accuracy depends on sources.")
